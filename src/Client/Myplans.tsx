...
const Myplans: React.FC = () => {
    ...
    const getHandle = async () => {
        try {
            ...
            const response = await axios.get(`https://sk-smoky.vercel.app/api/user/getAllInvestmentPlans/${userId}`, {
                headers: {
                    ...
                    {myPlans && myPlans.length > 0 ? (
                        <>
                            ...
                            <div className="w-full h-max flex flex-col mt-5 gap-6">
                                {myPlans.map((plan) => (
                                    <div key={plan._id} className="w-full h-max flex flex-col gap-2 overflow-y-auto">
